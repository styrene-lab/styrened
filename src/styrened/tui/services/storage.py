...
async def _detect_storage_macos() -> list[StorageDevice]:
    ...
    try:
        ...
        logger.warning("Failed to detect storage on macOS: %s", e)
        return _get_mock_storage()
...
async def _detect_storage_linux() -> list[StorageDevice]:
    ...
    try:
        ...
        for device in data.get("blockdevices", []):
            # Only consider removable devices (RM=1 or RM=true depending on lsblk version)
            rm_value = device.get("rm")
            is_removable = rm_value is True or rm_value == "1" or rm_value == 1
            if is_removable and device.get("type") == "disk":
        ...
        # Log error and return mock data for development
        logger.warning("Failed to detect storage on Linux: %s", e)
        return _get_mock_storage()
