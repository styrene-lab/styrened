...
class TestHorizontalScaling:
    ...
    async def test_scale_from_1_to_10_pods(
        ...
        k8s_cluster.helm_upgrade(
            ...
            set_values={"replicaCount": 5}
        )

        # Wait for new pods and discovery
        await asyncio.sleep(45)

        pods = k8s_cluster.get_pods(test_namespace, label=f"app={release_name}")
        assert len(pods) == 5
    ...
    async def test_scale_down_graceful_shutdown(
        self, k8s_cluster, test_namespace, styrened_stack
    ):
        ...
        k8s_cluster.helm_upgrade(
            ...
            "./helm/styrened-test",
            namespace=test_namespace,
            set_values={"replicaCount": 2}
        )

        await asyncio.sleep(30)

        # Verify only 2 pods remain
        remaining_pods = k8s_cluster.get_pods(
            test_namespace,
            label=f"app={release_name}"
        )
...
class TestResourceLimits:
    ...
    @pytest.mark.asyncio
    @pytest.mark.smoke
    async def test_cpu_throttling_handling(
        self, k8s_cluster, test_namespace, styrened_stack
    ):
...
"""
        ...
        # Verify no OOM kill
        events = k8s_cluster.get_pod_events(pods[0], test_namespace)
        oom_events = [e for e in events if "OOM" in e.get("reason", "")]
        assert len(oom_events) == 0

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_resource_requests_vs_limits(
        self, k8s_cluster, test_namespace, styrened_stack
    ):
        ...
        for pod in pods:
            # Get pod status
            status = k8s_cluster.get_pod_status(pod, test_namespace)
...
class TestNetworkBandwidth:
    ...
    @pytest.mark.slow
    @pytest.mark.comprehensive
    async def test_bandwidth_saturation(
        self, k8s_cluster, test_namespace, styrened_stack
    ):
...
"""
        ...
        for pod in pods[:3]:  # Use 3 pods as senders
            task = k8s_cluster.exec_in_pod_async(
                pod,
                ["python3", "-c", script]
            )
            tasks.append(task)
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_network_latency_impact(
        self, k8s_cluster, test_namespace, styrened_stack
    ):
        ...
        result = k8s_cluster.exec_in_pod(
            pods[0],
            ["ping", "-c", "3", f"{pods[1]}.{test_namespace}.svc.cluster.local"]
        )
        assert result.returncode == 0
...
class TestNodeAffinity:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_pods_spread_across_nodes(
        self, k8s_cluster, test_namespace, styrened_stack
    ):
        """Test that pods are distributed across available nodes.

        Scenario:
        1. Deploy 5 pods with anti-affinity
        ...
        node_assignments = {}
        for pod in pods:
            node = k8s_cluster.get_pod_node(pod, test_namespace)
        ...
        if num_nodes > 1:
            ...
            result = k8s_cluster.exec_in_pod(
                pod_node_0,
                ["ping", "-c", "2", f"{pod_node_1}.{test_namespace}.svc.cluster.local"]
            )
