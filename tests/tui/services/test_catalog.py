...
class TestValidateProfileHardware:
    def test_validate_valid_single(self, temp_catalog_dir):
        catalog = load_catalog(temp_catalog_dir)
        result = validate_profile_hardware(catalog, catalog.profiles["node"], ["rpi4"])
        assert result is True

    def test_validate_valid_multi(self, temp_catalog_dir):
        catalog = load_catalog(temp_catalog_dir)
        result = validate_profile_hardware(catalog, catalog.profiles["edge-router"], ["rpi4", "gl-opal"])
        assert result is True

    def test_validate_unverified(self, temp_catalog_dir):
        catalog = load_catalog(temp_catalog_dir)
        result = validate_profile_hardware(catalog, catalog.profiles["node"], ["gl-opal"])
        assert result is False

    def test_validate_missing_traits(self, temp_catalog_dir):
        catalog = load_catalog(temp_catalog_dir)
        result = validate_profile_hardware(catalog, catalog.profiles["workstation"], ["rpi4"])
        assert result is False

    def test_validate_valid_traits(self, temp_catalog_dir):
        catalog = load_catalog(temp_catalog_dir)
        result = validate_profile_hardware(catalog, catalog.profiles["workstation"], ["t100ta"])
        assert result is True

    def test_validate_invalid_id(self, temp_catalog_dir):
        catalog = load_catalog(temp_catalog_dir)
        result = validate_profile_hardware(catalog, catalog.profiles["node"], ["nonexistent"])
        assert result is False
