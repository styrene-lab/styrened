...
openspec_change: operator-interface-testing
---

# Operator Interface Testing Paths

## Overview

Automated integration tests that verify TUI operations against live Styrene peers — replacing manual operator walkthroughs with reproducible test paths. Built incrementally from the Test Peer Alpha foundation (containerized styrened peer via podman), eventually absorbing pieces from the existing bare-metal and K8s harnesses.
...
Start with organic point-to-point testing of each functional piece between the host TUI and a containerized test peer. Each verified interaction becomes a test case that can be automated. The test peer has known, deterministic config (display name, identity hash, auto-reply message, etc.) so all values in the TUI can be asserted.

## Test Peer Infrastructure

- **Test Peer Alpha**: containerized styrened (podman, ghcr.io/styrene-lab/styrened:latest) with TCP server on port 5252, port-forwarded to host
- Host daemon peers to 127.0.0.1:5252 via ~/.reticulum/config TCPClientInterface entry
- Container config passed via CONFIG_YAML env var, identity auto-generated (SKIP_IDENTITY_GEN=1, styrened creates its own)
- Auto-reply enabled: 'Roger that — Test Peer Alpha received your transmission. Standing by.'

## Automation Target

Textual's pilot testing framework + a test fixture that spins up containerized peers with known configs, then drives the TUI programmatically to verify each operation end-to-end. Eventually folds in RBAC/permissions model testing with peers configured at different trust tiers.
