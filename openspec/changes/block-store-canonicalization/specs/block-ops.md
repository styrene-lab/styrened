...
`block_peer()` and `unblock_peer()` are redesigned with `identity_hash` as the
canonical key. No compat shim. All callers updated. Atomic write order:
1. `peer_blocks` table (persistent, authoritative)
2. In-memory RBAC (immediate enforcement)
3. `contacts.blocked` flag (UI hint, best-effort)
