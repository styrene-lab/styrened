...
- [x] 1.1 Add `peer_blocks` table to `init_db()`:
...
- [x] 1.2 Migrate `contacts` table PK from `peer_hash` to `identity_hash`:
...
- [x] 1.3 Add startup backfill: for each contacts row, attempt
...
- [x] 2.1 Redesign `block_peer(identity_hash, lxmf_dest_hash=None, alias=None) -> bool`:
...
- [x] 2.2 Redesign `unblock_peer(identity_hash) -> bool`:
...
- [x] 2.5 Update `get_blocked_peers() -> list[dict]`:
...
- [x] 2.6 Remove `_load_blocklist()` (dead code, was never wired)

- [x] 2.7 Write unit tests (TDD — tests first):
...
- [x] 3.1 Add NodeStore lookup in `_handle_lxmf_message` before RBAC check:
  ```python
...
- [x] 4.1 `CmdBlockPeerRequest`: remove `peer_hash` field, add `identity_hash: str = ""`,
  add `lxmf_dest_hash: str = ""`, add `alias: str = ""`.
  Update `to_payload()` and `from_payload()`.

- [x] 4.2 `CmdUnblockPeerRequest`: remove `peer_hash` field, add `identity_hash: str = ""`.

- [x] 4.3 `QueryBlockedPeersRequest` response: ensure handler returns `identity_hash`-keyed dicts.

- [x] 4.4 IPC handler `handle_cmd_block_peer`: validate `identity_hash` not empty,
  call `svc.block_peer(identity_hash, lxmf_dest_hash, alias)`.
  NO peer_hash fallback resolution.

- [x] 4.5 IPC handler `handle_cmd_unblock_peer`: validate `identity_hash` not empty,
  call `svc.unblock_peer(identity_hash)`.

- [x] 4.6 `IPCClient.block_peer(identity_hash, lxmf_dest_hash="", alias="")` — rename param.
  `IPCClient.unblock_peer(identity_hash)` — rename param.

- [x] 4.7 `IPCBridge.block_peer(identity_hash, lxmf_dest_hash="", alias="")` — rename param.
  `IPCBridge.unblock_peer(identity_hash)` — rename param.

- [x] 4.8 `MeshDevice.identity` property: **delete entirely** (no deprecation warning).
...
- [x] 4.9 TUI `ConversationScreen.action_block_peer`: pass `device.identity_hash` to
  `bridge.block_peer(identity_hash=..., lxmf_dest_hash=device.lxmf_destination_hash)`.

- [x] 4.10 TUI `ChatWidget._block_peer_async`: same — use `identity_hash` not `peer_hash`
...
- [x] All 4 groups merged and green on main
- [x] `just test-unit` passes (target: no regression from current count)
- [x] `block_peer("id_abc")` survives a daemon restart — blocks still enforced after restart
- [x] `message.source_hash` from a blocked peer is dropped using identity_hash RBAC check
- [x] No `peer_hash` key appears in any IPC block/unblock message payload
- [x] `MeshDevice().identity` raises AttributeError
- [x] Version bumped to `0.16.0`, tagged `v0.16.0`, pushed
