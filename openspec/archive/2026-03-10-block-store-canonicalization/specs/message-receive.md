...
`_handle_lxmf_message` currently checks `message.source_hash` (an LXMF destination hash)
directly against `RBACPolicy.resolve_role()`. Blocks keyed on identity_hash in RBAC
never match LXMF dest hashes — the check is effectively broken for YAML-configured blocks.
Fix: resolve LXMF dest hash → identity_hash via NodeStore before the RBAC check.

---

## CHANGED: _handle_lxmf_message RBAC check
