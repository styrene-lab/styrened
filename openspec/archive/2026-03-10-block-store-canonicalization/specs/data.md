...
The current `contacts` table uses an LXMF destination hash as its PK (`peer_hash`).
Runtime blocks written via `block_peer()` do not survive daemon restarts.
`peer_blocks` is the new authoritative runtime block store, keyed on `rns_identity_hash`.

---

## CHANGED: contacts table schema
