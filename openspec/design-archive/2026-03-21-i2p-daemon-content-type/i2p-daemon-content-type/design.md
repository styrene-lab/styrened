...
- File scope is STALE — references deleted Python daemon modules (page_browser.py, ipc/handlers.py, rpc/server.py). Implementation must target the Rust daemon (styrened-rs) instead. Design decisions remain valid; file paths need Rust equivalents.
